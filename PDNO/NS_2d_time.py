...
class SpectralConv2d_fast(nn.Module):
    def __init__(self, in_channels, out_channels, num_layer, num_hidden, symbol_act, net_out):
        ...
        if net_out:
            self.net_out = Net(d_in=2, d_out=in_channels * out_channels, num_layer=num_layer, num_hidden=num_hidden, act=symbol_act)
    ...
    def get_grid(self, shape):
        ...
        mx, my = torch.meshgrid(torch.linspace(0, 1, mx), torch.linspace(0, 1, my))
    ...
    def get_grid_freq(self, shape):
        ...
        mx, my = torch.meshgrid(torch.fft.fftfreq(mx, d=1), torch.fft.rfftfreq(my, d=1))
        mx, my = mx.to(device), my.to(device)
        return torch.stack([mx, my], dim=-1)
...
if __name__ == "__main__":
    ...
    if nu == 1e-5:
        ...
        ntrain = 1000
        ntest = 200
        u = torch.load("data/ns_V1000_N5000_T50.mat")
    ...
    testloader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(test_a, test_u), batch_size=batch_size, shuffle=False)
