...
path = "ns_fourier_3d_rnn_V10000_T50_N" + str(ntrain) + "_ep" + str(epochs) + "_m" + str(modes) + "_w" + str(width)
...
gridx = gridx.reshape(1, S, 1, 1, 1).repeat([1, 1, S, step, 1])  # N, X, Y, T, C
gridy = torch.tensor(np.linspace(0, 1, S), dtype=torch.float)
gridy = gridy.reshape(1, 1, S, 1, 1).repeat([1, S, 1, step, 1])
gridt = torch.tensor(np.linspace(0, 1, T_in + T + 1)[1 : step + 1], dtype=torch.float)
...
test_a = torch.cat(
    (gridx.repeat([ntest, 1, 1, 1, 1]), gridy.repeat([ntest, 1, 1, 1, 1]), gridt.repeat([ntest, 1, 1, 1, 1]), test_a),
    dim=-1,
)
...
for ep in range(epochs):
    ...
    for xx, yy in train_loader:
        ...
        for t in range(0, T, step):
            ...
            if t == 0:
                pred = im.squeeze()  # N, X, Y, T
            else:
                pred = torch.cat((pred, im.squeeze()), -1)

            im = im.reshape(batch_size, S, S, step, 1)
            gridt = torch.tensor(np.linspace(0, 1, T_in + T + 1)[1 + t : 1 + t + step], dtype=torch.float)
            gridt = gridt.reshape(1, 1, 1, step, 1).repeat([1, S, S, 1, 1])
            ...
            im = torch.cat(
                (
                    ...
                    gridt.repeat([batch_size, 1, 1, 1, 1]),
                    im,
                ),
                dim=-1,
            )
            # print("im shape: ", im.shape)
            # print(xx[..., step:, :].shape, im.shape)
