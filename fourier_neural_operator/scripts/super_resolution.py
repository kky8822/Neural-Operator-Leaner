...
def compl_mul3d(a, b):
    ...
    return torch.stack([op(a[..., 0], b[..., 0]) - op(a[..., 1], b[..., 1]), op(a[..., 1], b[..., 0]) + op(a[..., 0], b[..., 1])], dim=-1)
...
print(reader.read_field("u").shape, test_a.shape, test_u.shape)
...
test_a = torch.cat((gridx.repeat([ntest, 1, 1, 1, 1]), gridy.repeat([ntest, 1, 1, 1, 1]), gridt.repeat([ntest, 1, 1, 1, 1]), test_a), dim=-1)
