...
path = 'ns_fourier_3d_rnn_extrapolation_10step_V10000_T50_N'+str(ntrain)+'_ep' + str(epochs) + '_m' + str(modes) + '_w' + str(width)
...
extrapolation_a = reader.read_field('u')[-ntest:,::sub,::sub,T_start:T_in]
extrapolation_u = reader.read_field('u')[-ntest:,::sub,::sub,T_in:T_in+T+T_ext]
...
extrapolation_a = extrapolation_a.reshape(ntest,S,S,step,1)
...
extrapolation_a = torch.cat((gridx.repeat([ntest,1,1,1,1]), gridy.repeat([ntest,1,1,1,1]),
                       gridt.repeat([ntest,1,1,1,1]), extrapolation_a), dim=-1)

print("train dataset shape: ", train_a.shape, train_u.shape)
print("test dataset shape: ", test_a.shape, test_u.shape)
train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(train_a, train_u), batch_size=batch_size, shuffle=True)
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(test_a, test_u), batch_size=batch_size, shuffle=False)
extrapolation_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(extrapolation_a, extrapolation_u), batch_size=batch_size, shuffle=False)
...
# model = torch.load('./model/ns_fourier_3d_rnn_extrapolation_10step_V10000_T50_N1000_ep500_m12_w20')
...
# gridt = gridt.to(device)
# with torch.no_grad():
#     for i, (xx, yy) in enumerate(extrapolation_loader):
#         xx = xx.to(device)
#         yy = yy.to(device)
#         # print(i, xx.shape, yy.shape)
        
#         pred = torch.zeros([batch_size,64,64,40])
#         u = torch.zeros([batch_size,64,64,40])
#         for t in range(0, T+T_ext, step):
#             y = yy[..., t:t + step]
#             # print(xx.shape)
#             im = model(xx)
...
#             xx = torch.cat([xx[..., step:, :], im], -2)
#         pred_tot[i*batch_size:(i+1)*batch_size,:,:,:] = pred
#         u_tot[i*batch_size:(i+1)*batch_size,:,:,:] = u
