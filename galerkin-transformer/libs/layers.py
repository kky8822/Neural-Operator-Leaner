...
class GraphAttention(nn.Module):
    ...
    def forward(self, node, adj):
        ...
        a_input = torch.cat(
            [
                h.repeat(1, 1, seq_len).view(bsz, seq_len * seq_len, -1),
                h.repeat(1, seq_len, 1),
            ],
            dim=2,
        )
...
class EdgeEncoder(nn.Module):
    def __init__(self, out_dim: int, edge_feats: int, raw_laplacian=None):
...
class Conv2dEncoder(nn.Module):
    ...
    def __init__(
        ...
        activation_type = default(activation_type, "silu")
        self.conv0 = Conv2dResBlock(in_dim, out_dim, kernel_size=kernel_size, padding=padding, residual=residual)
        self.conv1 = Conv2dResBlock(
            out_dim,
            conv_dim0,
            kernel_size=kernel_size,
            padding=padding1,
        ...
        self.conv3 = Conv2dResBlock(conv_dim1, conv_dim2, kernel_size=kernel_size, residual=residual)
        self.pool0 = nn.AvgPool2d(kernel_size=scaling_factor, stride=scaling_factor)
        self.pool1 = nn.AvgPool2d(kernel_size=scaling_factor, stride=scaling_factor)
        self.activation = nn.SiLU() if activation_type == "silu" else nn.ReLU()
...
class Interp2dEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.is_scale_factor = isinstance(interp_size[0], float) and isinstance(interp_size[1], float)
        self.conv0 = Conv2dResBlock(
            in_dim,
            out_dim,
            kernel_size=kernel_size,
            padding=padding,
    ...
    def forward(self, x):
        ...
        #     x = F.interpolate(
        #         x,
        #         scale_factor=self.interp_size[0],
        #         mode="bilinear",
        #         recompute_scale_factor=True,
        #         align_corners=True,
        #     )
        ...
        #     out = F.interpolate(
        #         out,
        #         scale_factor=self.interp_size[1],
        #         mode="bilinear",
        #         recompute_scale_factor=True,
        #         align_corners=True,
        #     )
        # else:
        #     out = F.interpolate(
        #         out, size=self.interp_size[1], mode="bilinear", align_corners=True
        #     )
...
class Interp2dUpsample(nn.Module):
    ...
    def forward(self, x):
        x = F.interpolate(x, size=self.interp_size[0], mode=self.interp_mode, align_corners=True)
        if self.conv_block:
            x = self.conv(x)
        x = F.interpolate(x, size=self.interp_size[1], mode=self.interp_mode, align_corners=True)
        return x


def attention(query, key, value, mask=None, dropout=None, weight=None, attention_type="softmax"):
    """
    Simplified from
    ...
    if attention_type == "cosine":
        p_attn = F.cosine_similarity(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    else:
...
def linear_attention(query, key, value, mask=None, dropout=None, attention_type="galerkin"):
    """
    ...
    # print(
    #     "LIN_ATTN key_T, value, score: ",
    #     key.transpose_(-2, -1).shape,
    #     value.shape,
    #     scores.shape,
    # )
    ...
    p_attn = scores / seq_len
    # print(
    #     "LIN_ATTN p_attn, scores, seq_len: ",
    #     p_attn.shape,
    #     scores.shape,
    #     seq_len,
    # )
...
def causal_linear_attn(query, key, value, kv_mask=None, dropout=None, eps=1e-7):
    ...
    if kv_mask is not None:
        ...
        key = key.masked_fill_(~mask, 0.0)
        value = value.masked_fill_(~mask, 0.0)
        del mask
    ...
    D_inv = 1.0 / torch.einsum("bhud,bhund->bhun", b_k_cumsum + eps, b_q)
...
class SimpleAttention(nn.Module):
    ...
    def forward(self, query, key, value, pos=None, mask=None, weight=None):
        ...
        if self.add_norm:
            if self.attention_type in ["linear", "galerkin", "global"]:
                ...
                key = torch.stack(
                    [norm(x) for norm, x in zip(self.norm_K, (key[:, i, ...] for i in range(self.n_head)))],
                    dim=1,
                )
                value = torch.stack(
                    [norm(x) for norm, x in zip(self.norm_V, (value[:, i, ...] for i in range(self.n_head)))],
                    dim=1,
                )
                ...
                query = torch.stack(
                    [norm(x) for norm, x in zip(self.norm_Q, (query[:, i, ...] for i in range(self.n_head)))],
                    dim=1,
                )
        ...
        if pos is not None and self.pos_dim > 0:
            ...
            # print("pos, query: ", pos.shape, query.shape)
        ...
        if self.attention_type in ["linear", "galerkin", "global"]:
            ...
            x, self.attn_weight = causal_linear_attn(query, key, value, mask=mask, dropout=self.dropout)
        else:
            x, self.attn_weight = attention(
                query,
                key,
                value,
                mask=mask,
        ...
        for param in self.linears.parameters():
            if param.ndim > 1:
                ...
                if self.diagonal_weight > 0.0:
                    param.data += self.diagonal_weight * torch.diag(torch.ones(param.size(-1), dtype=torch.float))
    ...
    def _get_norm(self, eps):
        if self.attention_type in ["linear", "galerkin", "global"]:
            ...
            if self.norm_type == "instance":
                self.norm_K = self._get_instancenorm(self.d_k, self.n_head, eps=eps, affine=True)
                self.norm_Q = self._get_instancenorm(self.d_k, self.n_head, eps=eps, affine=True)
            elif self.norm_type == "layer":
                self.norm_K = self._get_layernorm(self.d_k, self.n_head, eps=eps)
    ...
    def _get_layernorm(normalized_dim, n_head, **kwargs):
        return nn.ModuleList([copy.deepcopy(nn.LayerNorm(normalized_dim, **kwargs)) for _ in range(n_head)])

    @staticmethod
    def _get_instancenorm(normalized_dim, n_head, **kwargs):
        return nn.ModuleList([copy.deepcopy(nn.InstanceNorm1d(normalized_dim, **kwargs)) for _ in range(n_head)])


class SimpleSpatioTemporalAttention(nn.Module):
    """
    The attention is using a vanilla (QK^T)V or Q(K^T V) with no softmax
    For an encoder layer, the tensor size is slighly different from the official pytorch implementation
    ...
    def __init__(
        ...
        super(SimpleSpatioTemporalAttention, self).__init__()
        assert d_model % n_head == 0
        self.attention_type = attention_type
        self.d_k = d_model // n_head
        self.n_head = n_head
    ...
    def forward(self, query, key, value, pos=None, temp_out=None, mask=None, weight=None):
        ...
        if pos is not None and self.pos_dim > 0:
            ...
            # print("pos, temp, query: ", pos.shape, temp_out.shape, query.shape)
        ...
        out_dim = (
            self.n_head * self.d_k if pos is None and temp_out is None else self.n_head * (self.d_k + self.pos_dim)
        )
        att_output = x.transpose(1, 2).contiguous().view(bsz, -1, out_dim)
...
class BulkRegressor(nn.Module):
    ...
    def __init__(
        ...
        self.regressor = nn.ModuleList([copy.deepcopy(freq_out) for _ in range(n_targets)])
        self.dropout = nn.Dropout(dropout)
...
class SpectralConv2d(nn.Module):
    def __init__(
        ...
        self.fourier_weight = nn.ParameterList(
            [Parameter(torch.FloatTensor(in_dim, out_dim, modes, modes, 2)) for _ in range(2)]
        )
    ...
    def forward(self, x):
        ...
        out_ft = torch.zeros(batch_size, out_dim, n, n // 2 + 1, 2, device=x.device)
