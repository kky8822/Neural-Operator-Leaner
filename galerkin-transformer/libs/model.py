...
from galerkin_transformer.model import FourierTransformerEncoderLayer
...
class SimpleTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, pos=None, weight=None):
        ...
        # Attention # encoder no mask
        if self.de_solver == "euler":
            if pos is not None and self.pos_dim > 0:
                F1_output, F1_weight = self.attn(x, x, x, pos=pos, weight=weight)
            else:
                F1_output, F1_weight = self.attn(x, x, x, weight=weight)
        elif self.de_solver == "rk2":
            if pos is not None and self.pos_dim > 0:
                F1_output, F1_weight = self.attn(x, x, x, pos=pos, weight=weight)
                F2_output, F2_weight = self.attn(x + F1_output, x + F1_output, x + F1_output, pos=pos, weight=weight)
            else:
                F1_output, F1_weight = self.attn(x, x, x, weight=weight)
                F2_output, F2_weight = self.attn(x + F1_output, x + F1_output, x + F1_output, weight=weight)
        elif self.de_solver == "rk4":
            if pos is not None and self.pos_dim > 0:
                F1_output, F1_weight = self.attn(x, x, x, pos=pos, weight=weight)
                F2_output, F2_weight = self.attn(
                    x + F1_output * 0.5,
                    x + F1_output * 0.5,
                    x + F1_output * 0.5,
                    pos=pos,
                ...
                F4_output, F4_weight = self.attn(x + F3_output, x + F3_output, x + F3_output, pos=pos, weight=weight)
            else:
                F1_output, F1_weight = self.attn(x, x, x, weight=weight)
                F2_output, F2_weight = self.attn(
                    x + F1_output * 0.5,
                ...
                F4_output, F4_weight = self.attn(x + F3_output, x + F3_output, x + F3_output, weight=weight)
        else:
            print("Please check de_solver tag")

        # Residual connection
        if self.de_solver == "euler":
            if self.residual_type in ["add", "plus"] or self.residual_type is None:
                x = x + self.dropout1(F1_output)
            else:
                x = x - self.dropout1(F1_output)
        elif self.de_solver == "rk2":
            if self.residual_type in ["add", "plus"] or self.residual_type is None:
                x = x + self.dropout1(F1_output) * 0.5 + self.dropout1(F2_output) * 0.5
            else:
                x = x - self.dropout1(F1_output) * 0.5 - self.dropout1(F2_output) * 0.5
        elif self.de_solver == "rk4":
            if self.residual_type in ["add", "plus"] or self.residual_type is None:
                x = (
                    x
                    + self.dropout1(F1_output) * 1 / 6
                    + self.dropout1(F2_output) * 1 / 3
                    + self.dropout1(F3_output) * 1 / 3
                    + self.dropout1(F4_output) * 1 / 6
                )
            else:
                x = (
                    x
...
class SimpleTransformerSpatioTemporalEncoderLayer(nn.Module):
    def __init__(
        self,
        d_model=96,
        pos_dim=1,
        ...
        super(SimpleTransformerSpatioTemporalEncoderLayer, self).__init__()
        ...
        #     dropout=dropout,
        # )
        self.attn = SimpleSpatioTemporalAttention(
            n_head=n_head,
            d_model=d_model,
            attention_type=attention_type,
            diagonal_weight=diagonal_weight,
    ...
    def forward(self, x, pos=None, weight=None):
        ...
        # Attention # encoder no mask
        # print("TR ATTN in: ", x.shape)
        if self.de_solver == "euler":
            if pos is not None and self.pos_dim > 0:
                F1_output, F1_weight = self.attn(x, x, x, pos=pos, weight=weight)
                # print("TR ATTN out: ", F1_output.shape)
            else:
                F1_output, F1_weight = self.attn(x, x, x, weight=weight)
        elif self.de_solver == "rk2":
            if pos is not None and self.pos_dim > 0:
        ...
        # print("TR RESCON in: ", x.shape, F1_output.shape)
        # Residual connection
        if self.de_solver == "euler":
            if self.residual_type in ["add", "plus"] or self.residual_type is None:
                x = x + self.dropout1(F1_output)
...
class GalerkinTransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.multihead_attn = SimpleAttention(
            nhead,
            d_model,
...
class _TransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if (src_mask is None) or (src_key_padding_mask is None):
            ...
            src2, attn_weight = self.self_attn(src, src, src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask)
...
class GCN(nn.Module):
    def __init__(
        ...
        """
        self.edge_learner = EdgeEncoder(out_dim=out_features, edge_feats=edge_feats, raw_laplacian=raw_laplacian)
        self.gcn_layer0 = GraphConvolution(
            in_features=node_feats,  # hard coded
...
class GAT(nn.Module):
    def __init__(
        ...
        """
        self.gat_layer0 = GraphAttention(
...
class SimpleBlock2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.fc0(x)
        # print(x.shape)
        x = x.permute(0, 4, 1, 2, 3)  # N, F, H, W, T
        # print(x.shape)

        x1 = self.conv0(x)
        # print(x1.shape)
        # print(x.shape, batchsize, self.width)
        ...
        x = x.permute(0, 2, 3, 4, 1)  # N, H, W, T, F
        x = self.fc1(x)
        x = F.relu(x)
        x = self.fc2(x)
        return x
...
class DownScaler(nn.Module):
    def __init__(
        ...
        if downsample_mode == "conv":
            ...
            self.downsample = Interp2dEncoder(
                in_dim=in_dim,
                out_dim=out_dim,
...
class UpScaler(nn.Module):
    def __init__(
        ...
        if upsample_mode in ["conv", "deconv"]:
            ...
            self.upsample = Interp2dUpsample(
                in_dim=in_dim,
                out_dim=out_dim,
...
class SimpleTransformer(nn.Module):
    ...
    def forward(self, node, edge, pos, grid=None, weight=None):
        ...
        return dict(preds=x, preds_freq=x_freq, preds_latent=x_latent, attn_weights=attn_weights)
    ...
    def _get_freq_regressor(self):
        if self.bulk_regression:
            self.freq_regressor = BulkRegressor(
                in_dim=self.seq_len,
...
class FourierTransformer2D(nn.Module):
    ...
    def forward(self, node, edge, pos, grid, weight=None, boundary_value=None):
        ...
        if not self.downscaler_size:
            node = torch.cat([node, pos.contiguous().view(bsz, n_s, n_s, -1)], dim=-1)
        ...
        return dict(preds=x, preds_latent=x_latent, attn_weights=attn_weights)
    ...
    def _get_scaler(self):
        if self.downscaler_size:
            ...
            self.downscaler = Identity(
                in_features=self.node_feats + self.spacial_dim,
                out_features=self.n_hidden,
            )
    ...
    def _get_encoder(self):
        if self.attention_type in self.attention_types:
            ...
            encoder_layer = TransformerEncoderLayer(
                d_model=self.n_hidden + self.pos_dim * self.n_head,
                nhead=self.n_head,
                dim_feedforward=self.dim_feedforward,
                dropout=self.encoder_dropout,
                batch_first=True,
...
class FourierTransformer3DLite(nn.Module):
    ...
    def forward(self, node, edge, pos, grid=None):
        ...
        n_grid = grid.size(1)
        t_grid = grid.size(3)
        node = node.unsqueeze(-2).repeat(1, 1, 1, t_grid, 1)
        node = torch.cat([node.view(bsz, -1, input_dim), pos], dim=-1)
        x = self.feat_extract(node, edge)
        ...
        x = self.dpo(x)
        x = x.view(bsz, n_grid, n_grid, t_grid, -1).permute(0, 1, 2, 4, 3)

        x_out = torch.zeros(bsz, n_grid, n_grid, t_grid).to(self.device)
        for t in range(t_grid):
            x_out[..., t] = self.regressor(x[..., t]).squeeze()  # N, H, W, F

        return dict(preds=x_out, preds_freq=None, preds_latent=None, attn_weights=None)
    ...
    def _get_encoder(self):
        encoder_layer = SimpleTransformerSpatioTemporalEncoderLayer(
            d_model=self.n_hidden,
            n_head=self.n_head,
            dim_feedforward=self.dim_feedforward,
            layer_norm=self.layer_norm,
    ...
    def _get_regressor(self):
        ...
        # self.regressor = Net2d(modes=self.fourier_modes, width=self.n_hidden)
