...
FIG_PATH = default(os.environ.get("FIG_PATH"), os.path.join(os.path.dirname(SRC_ROOT), "figures"))
EPOCH_SCHEDULERS = [
    "ReduceLROnPlateau",
    "StepLR",
    "MultiplicativeLR",
...
def showsolution(node, elem, u, **kwargs):
    ...
    if "template" not in kwargs.keys():
        fig.update_layout(template="plotly_dark", margin=dict(l=5, r=5, t=5, b=5), **kwargs)
    else:
...
def showcontour(z, **kwargs):
    ...
    if "template" not in kwargs.keys():
        fig.update_layout(template="plotly_dark", margin=dict(l=0, r=0, t=0, b=0), **kwargs)
    else:
        fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), **kwargs)
    fig.show()
    return fig
...
def get_args_2d(
    ...
    **kwargs,
    ...
    if inverse:
        ...
        parser = argparse.ArgumentParser(description="Example 2: Darcy interface flow")
...
def run_train(
    ...
    if tqdm_switch == True:
        with tqdm(total=end_epoch - start_epoch, disable=not tqdm_epoch) as pbar_ep:
            for epoch in range(start_epoch, end_epoch):
                ...
                if val_result["metric"].ndim == 0:
                    ...
                    desc = color(f"| val metric 1: {metric_0:.3e} ", color=Colors.blue)
                    desc += color(f"| val metric 2: {metric_1:.3e} ", color=Colors.blue)
                desc += color(
                    f"| best val: {best_val_metric:.3e} at epoch {best_val_epoch+1}",
                    color=Colors.yellow,
                )
                ...
                if not tqdm_epoch:
                    ...
                    if _loss_mean.ndim == 0:  # 1 target loss
                        desc_ep += color(f"| loss: {_loss_mean:.3e} ", color=Colors.green)
                    else:
                        for j in range(len(_loss_mean)):
                            if _loss_mean[j] > 0:
                                desc_ep += color(
                                    f"| loss {j}: {_loss_mean[j]:.3e} ",
                                    color=Colors.green,
                                )
                    desc_ep += desc
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            save_pickle(result, os.path.join(model_save_path, result_name))
            print(epoch, loss_train[-1], loss_val[-1])

    return result


class ProfileResult:
    def __init__(self, result_file, num_iters=1, cuda=True) -> None:
        """
        ...
        self.df = pd.read_csv(result_file, delim_whitespace=True, skiprows=range(5), header=None)
