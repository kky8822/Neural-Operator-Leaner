...
def cal_lossT(VPTR_Disc, fake_imgs, real_imgs, fake_feats, real_feats, lam_pc, lam_gan):
    ...
    if VPTR_Disc is not None:
        ...
        # T_MSE_loss = T_MSE_loss * 1e-10
        # T_GDL_loss = T_GDL_loss * 1e-10
...
def single_iter(
    ...
    if train_flag:
        ...
        pred_future_feats = VPTR_Transformer.NCE_projector(pred_future_feats.permute(0, 1, 3, 4, 2)).permute(
            0, 1, 4, 2, 3
        )
        ...
        with torch.no_grad():
            ...
            future_gt_feats = VPTR_Transformer.NCE_projector(future_gt_feats.permute(0, 1, 3, 4, 2)).permute(
                0, 1, 4, 2, 3
            )
            loss_T, T_GDL_loss, T_MSE_loss, T_PC_loss, loss_T_gan = cal_lossT(
                VPTR_Disc,
...
def NAR_show_samples(
    ...
    with torch.no_grad():
        ...
        pred_frames = torch.cat((past_frames[:, :, 0:pred_channels, ...], pred_future_frames), dim=1)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "ns":
        ...
        tensorboard_save_dir = Path("/kky/VPTR/VPTR_ckpts/0722_NS_10_NAR_MSEGDL_tensorboard")
        ...
        tensorboard_save_dir = Path("/kky/VPTR/VPTR_ckpts/0723_NSBD_10_NAR_MSEGDL_tensorboard")
        ...
        tensorboard_save_dir = Path("/kky/VPTR/VPTR_ckpts/0721_KTH_NAR_MSEGDL_tensorboard")
        ...
        ckpt_save_dir = Path("/kky/VPTR/VPTR_ckpts/0816_re_NSBDField_RK4_40_NAR_MSEGDL_ckpt")
        tensorboard_save_dir = Path("/kky/VPTR/VPTR_ckpts/0816_re_NSBDField_RK4_40_NAR_MSEGDL_tensorboard")
        ...
        ckpt_save_dir = Path("/kky/VPTR/VPTR_ckpts/0817_NSBDTotalMinMax_1to1_40_NAR_MSEGDL_ckpt")
        tensorboard_save_dir = Path("/kky/VPTR/VPTR_ckpts/0817_NSBDTotalMinMax_1to1_40_NAR_MSEGDL_tensorboard")
        resume_AE_ckpt = Path("/kky/VPTR/VPTR_ckpts/0817_NSBDTotalMinMax_1to1_40_ResNetAE_MSEGDLlsgan_ckpt").joinpath(
            "epoch_113.tar"
        )
        device = torch.device("cuda:0")
    ...
    for epoch in range(start_epoch + 1, start_epoch + epochs + 1):
        ...
        for idx, sample in enumerate(train_loader, 0):
            # past_frames, future_frames = sample
            # print(torch.max(past_frames), torch.max(future_frames))
