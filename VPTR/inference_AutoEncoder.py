...
if __name__ == "__main__":
    if sys.argv[1] == "nsbd":
        ...
        ckpt_save_dir = Path("/kky/VPTR/VPTR_ckpts/0813_NSBDTotalMinMax_4to4_40_ResNetAE_MSEGDLlsgan_ckpt")
        tensorboard_save_dir = Path(
            "/kky/VPTR/VPTR_ckpts/0813_NSBDTotalMinMax_4to4_40_ResNetAE_MSEGDLlsgan_tensorboard"
        )
        device = torch.device("cuda:0")
        data_set_name = "Navier-Stokes-total-BD"  # see utils.dataset
        dataset_dir = "/kky/VPTR/dataset/navier-stokes-total-12000"
        img_channels = 4  # 3 channels for BAIR datset
    ...
    columns = (
        ["t"] + ["psnr_" + str(i) for i in range(psnr.shape[1])] + ["ssim_" + str(i) for i in range(psnr.shape[1])]
    )
    data = np.hstack((psnr.numpy(), ssim.numpy()))
    data = np.hstack((np.arange(len(psnr)).reshape(-1, 1), data))

    df = pd.DataFrame(columns=columns, data=data)
    df.to_csv(ckpt_save_dir.joinpath("AE_test_metrics_" + visc_list[0] + ".csv"), index=False)
