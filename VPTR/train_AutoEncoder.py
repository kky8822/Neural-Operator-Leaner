...
if __name__ == "__main__":
    if sys.argv[1] == "nsbd":
        ...
        ckpt_save_dir = Path("/kky/VPTR/VPTR_ckpts/0817_NSBDTotalMinMax_1to1_40_ResNetAE_MSEGDLlsgan_ckpt")
        tensorboard_save_dir = Path(
            "/kky/VPTR/VPTR_ckpts/0817_NSBDTotalMinMax_1to1_40_ResNetAE_MSEGDLlsgan_tensorboard"
        )
        device = torch.device("cuda:0")
    ...
    resume_ckpt = ckpt_save_dir.joinpath("epoch_109.tar")
    # print(resume_ckpt)
    # resume_ckpt = None
    start_epoch = 0
    ...
    VPTR_Dec = VPTRDec(pred_channels, feat_dim=encC, n_downsampling=3, out_layer="Sigmoid").to(
        device
    )  # Sigmoid for MNIST, Tanh for KTH and BAIR
