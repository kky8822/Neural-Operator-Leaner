...
class VidHRFormerBlockEnc(nn.Module):
    ...
    def forward(self, x, local_window_pos_embed, temporal_pos_embed):
        ...
        if self.far:
            ...
            if self.rk == False:
                F1 = temporal_self_attention(q=x, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask)
                x = x + F1
            elif self.rk == "rk2":
                F1 = temporal_self_attention(q=x, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask)
                F2 = temporal_self_attention(q=x + F1, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask)
                x = x + 1 / 2 * (F1 + F2)
            elif self.rk == "rk4":
                F1 = temporal_self_attention(q=x, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask)
                ...
                F4 = temporal_self_attention(q=x + F3, temporal_pos_embed=temporal_pos_embed, attn_mask=attn_mask)
...
class TemporalSpatialLocalMultiheadAttention(nn.Module):
    ...
    def forward(self, memory, query, TS_local_pos_embed):
        ...
        # attention
        out = self.attn(
            query=query_permute + TS_local_pos_embed[T1 : T1 + T2, ...].flatten(0, 2)[:, None, :],
